...
Future<void> main() async {
  final extractionService = ClinicalExtractionService(
  ...
  const dataset = <EvaluationSample>[
    EvaluationSample(
      transcript:
          'Fever with cough for two days. Advise CBC. Start paracetamol 650 mg thrice daily for 5 days.',
    ...
    EvaluationSample(
      transcript:
          'Severe throat pain with fever. Prescribe cetirizine 10 mg once daily for 3 days.',
  ...
  final report = EvaluationReport(
    complaintPrecision: complaintPredicted == 0
        ? 0
        : complaintTruePositive / complaintPredicted,
    diagnosisRecall:
        diagnosisExpected == 0 ? 0 : diagnosisTruePositive / diagnosisExpected,
    prescriptionAccuracy: prescriptionCorrect / dataset.length,
  );

  stdout.writeln(
      'Complaint precision: ${(report.complaintPrecision * 100).toStringAsFixed(1)}%');
  stdout.writeln(
      'Diagnosis recall: ${(report.diagnosisRecall * 100).toStringAsFixed(1)}%');
  stdout.writeln(
      'Prescription accuracy: ${(report.prescriptionAccuracy * 100).toStringAsFixed(1)}%');
}
